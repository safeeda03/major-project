...
const Nutrition = () => {
  ...
  useEffect(() => {
    ...
    const loadChildren = async () => { setRecordsLoading(true); setError(''); try {
      const beneficiaries = await beneficiaryAPI.getAll(); const parentIds = new Set([user?.id, user?.user_id, user?._id].filter(Boolean));
      const linkedChildren = beneficiaries.filter((beneficiary) => parentIds.has(beneficiary.parent_id)); setChildren(linkedChildren); setSelectedChildId(linkedChildren[0]?.beneficiary_id || '');
